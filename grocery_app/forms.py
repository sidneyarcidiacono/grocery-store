...
class GroceryStoreForm(FlaskForm):
    ...
    title = StringField(
        "Title", validators=[DataRequired(), Length(min=3, max=25)]
    )
    address = StringField(
        "Address", validators=[DataRequired(), Length(min=3, max=80)]
    )
...
class GroceryItemForm(FlaskForm):
    ...
    price = FloatField(
        "Price", validators=[DataRequired(), NumberRange(min=0)]
    )
    category = SelectField(
        "Category",
        choices=["PRODUCE", "DELI", "BAKERY", "PANTRY", "FROZEN", "OTHER"],
